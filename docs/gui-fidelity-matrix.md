...
| Slot/facet inspection | Patent and application reports emphasize local, inherited, and combined values plus facets. | `implemented` | Were local/inherited/combined values shown together in KEE's browser? | Improve slot table to show local/inherited/combined columns in the browser pane, not only inspector detail. |
...
| KEEpictures | AIAI and NASA TEXSYS/MTK describe mouse/menu graphics, graphical model-building, and slot-driven animation. | `missing` | What did editing a KEEpicture look like? | Add a small graphical object model before trying to mimic a full editor. |
