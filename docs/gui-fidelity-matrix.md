...
| ASKE-style Common Windows workspace | ASKE Chapter V gives a concrete KEE 3.1 / Unisys Explorer application UI: Aske and Rulemaker interfaces, icon actions, Interaction Window, Notebook, Display Window, Rule/Context/Class display windows, Rule Editing Window, Central Concepts Window, and left/middle mouse actions. | `missing` | Does this reflect normal KEE/Common Windows idiom or mostly one application's conventions? | Build a small ASKE-inspired reviewer demo after the source PDF is manually mirrored, keeping it clearly separate from the AUV image-panel demo. |
...
The compact reconstructed desktop context and Desktop roster now also switch
between reconstructed Lisp Listener, Typescript, and Prompt transcript panes
for the generated session. The AUV demo presents its mission-selection,
parameter-entry, and monitoring panels as an image-panel window deck.
...
- Was the ASKE Aske/Rulemaker split familiar KEE/Common Windows practice, or
  mainly an application-specific knowledge-acquisition interface?
