...
(defun write-viewer-script (stream)
  (write-html-lines
   ...
   (list "</script>"
         ...
         "function makeAgendaGroup(agenda = null, world = null) { return { agenda, world: world || agenda?.world || null, matches: [], fires: [] }; }"
         "function agendaGroups(rows) { const groups = []; let current = null; rows.filter(agendaTraceP).forEach(event => { const kind = traceKind(event); if (kind === 'agenda') { current = makeAgendaGroup(event); groups.push(current); return; } const world = event.world || null; if (!current || current.world !== world) { current = makeAgendaGroup(null, world); groups.push(current); } if (kind === 'rule-match') current.matches.push(event); if (kind === 'rule-fire') current.fires.push(event); }); return groups.filter(group => group.agenda || group.matches.length || group.fires.length); }"
         "function sameTraceBindings(left, right) { return JSON.stringify(left || []) === JSON.stringify(right || []); }"
         "function sameRuleActivation(left, right) { return left.rule === right.rule && left.world === right.world && sameTraceBindings(left.bindings, right.bindings); }"
         ...
         "function renderAgendaDrilldown(candidate, group) { const conditions = agendaCandidateConditions(candidate, group); const action = agendaCandidateAction(candidate, group); const lines = []; if (conditions.length) lines.push(agendaDrillLine('conditions', conditions.map(traceValueText).join(' | '))); if (action) lines.push(agendaDrillLine('action', traceValueText(action))); return lines.length ? `<div class='agenda-drilldown'>${lines.join('')}</div>` : ''; }"
         "function renderAgendaCandidate(candidate, group) { const stateName = agendaCandidateState(candidate, group); const bindings = traceBindings(candidate); const classes = ['agenda-candidate', stateName]; if (candidate.id === state.traceFocusId) classes.push('focused'); return `<div class='${classes.join(' ')}' data-trace-id='${esc(candidate.id)}'><span class='agenda-status'>${stateName === 'fired' ? 'fired' : 'match'}</span>${refButton('unit', candidate.rule, DATA.units.kb)}${bindings ? ` <span class='code-text'>${esc(bindings)}</span>` : ''}${renderAgendaDrilldown(candidate, group)}</div>`; }"
         "function renderAgendaGroup(group) { const candidates = agendaCandidates(group).slice(-10); const title = group.agenda ? `Agenda ${refButton('unit', group.agenda.ruleClass, DATA.units.kb)}` : 'Conflict Set'; const world = group.world ? `world ${group.world}` : 'global'; const body = candidates.length ? `<div class='agenda-list'>${candidates.map(candidate => renderAgendaCandidate(candidate, group)).join('')}</div>` : `<p class='empty'>No candidates</p>`; return `<div class='agenda-pass'><div class='agenda-head'><strong>${title}</strong><span class='count'>${esc(world)} / ${esc(agendaSummary(group, candidates))}</span></div>${body}</div>`; }"
         ...
         "function renderAgendaPane(rows) { const groups = agendaGroups(rows).slice(-5).reverse(); return groups.length ? `${renderAgendaControls(rows)}<div class='agenda-board'>${groups.map(renderAgendaGroup).join('')}</div>` : ''; }"
