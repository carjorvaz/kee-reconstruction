...
(defun write-viewer-script (stream)
  (write-html-lines
   ...
   (list "</script>"
         ...
         "function causalEffectLabel(event) { const kind = traceKind(event); if (kind === 'world-slot-write') return `slot ${event.unit}.${event.slot}`; if (kind === 'world-branch') return `branch ${event.world || 'NIL'}`; if (kind === 'nogood') return `nogood ${event.world || 'NIL'}`; if (kind === 'contradiction') return `contradiction ${event.world || 'NIL'}`; return kind; }"
