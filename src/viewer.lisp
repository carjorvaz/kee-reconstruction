...
(defun write-viewer-script (stream)
  (write-html-lines
   ...
   (list "</script>"
         ...
         "  state.traceQuery = '';"
         "  state.traceKind = 'all';"
         "  state.traceFamily = kind === 'agenda' ? 'rules' : kind === 'kee-pictures' ? 'pictures' : 'all';"
         "  state.traceScope = kind === 'kee-pictures' ? 'all' : 'selected';"
         ...
         "}"
         "function renderReviewTour() { const items = [['units', 'Units'], ['rules', 'Rules'], ['worlds', 'Worlds'], ['agenda', 'Agenda'], ['xref', 'Rule Xref'], ['kee-pictures', 'KEEpictures'], ['active-images', 'ActiveImages']]; reviewTour.innerHTML = `<h3>Review Tour</h3><div class='tour-buttons'>${items.map(([kind, label]) => `<button type='button' data-review-tour='${kind}' ${reviewTourAvailable(kind) ? '' : 'disabled'}>${label}</button>`).join('')}</div>`; }"
         ...
         "function renderPictureBrowser() { const pictures = keePictures(); if (!pictures.length) { pictureBrowser.innerHTML = ''; return; } const selected = selectedPicture(); state.pictureName = selected.name; const tabs = pictures.map(picture => `<button type='button' data-picture-name='${esc(picture.name)}' class='${picture.name === selected.name ? 'active' : ''}'>${esc(picture.label || picture.name)}</button>`).join(''); pictureBrowser.innerHTML = `<section class='browser-section'><h3>KEEpictures</h3><div class='picture-tabs'>${tabs}</div><div class='kee-picture-preview'>${selected.svg || ''}</div>${pictureViewportSummary(selected)}</section>`; }"
         ...
         "function traceFamily(event) { const kind = traceKind(event); if (kind.startsWith('method-')) return 'methods'; if (kind.startsWith('rule-') || kind === 'agenda') return 'rules'; if (kind.startsWith('world-')) return 'worlds'; if (kind.startsWith('picture-')) return 'pictures'; if (kind === 'nogood' || kind === 'contradiction') return 'problems'; return 'other'; }"
         ...
         "function traceMapEventP(event) { return ['agenda', 'rule-match', 'rule-fire', 'world-create', 'world-branch', 'world-label-retract', 'picture-mouse', 'nogood', 'contradiction'].includes(traceKind(event)); }"
         "function traceMapLane(event) { const kind = traceKind(event); if (kind.startsWith('world-')) return { name: 'worlds', y: 42 }; if (kind.startsWith('picture-')) return { name: 'pictures', y: 132 }; if (kind === 'nogood' || kind === 'contradiction') return { name: 'problems', y: 176 }; return { name: 'rules', y: 86 }; }"
         "function traceMapLabel(event) { const kind = traceKind(event); if (kind === 'agenda') return event.ruleClass || kind; if (kind.startsWith('rule-') || kind === 'nogood') return event.rule || kind; if (kind.startsWith('world-') || kind === 'contradiction') return event.world || kind; if (kind.startsWith('picture-')) return event.item || event.picture || kind; return event.unit || kind; }"
