...
(defun write-viewer-head (stream title)
  ...
  (write-html-lines
   ...
   (list "<style>"
         ...
         ".trace-map-controls { display: grid; grid-template-columns: auto auto auto minmax(0, 1fr); gap: 6px; align-items: center; margin-bottom: 10px; }"
...
(defun write-viewer-script (stream)
  (write-html-lines
   ...
   (list "</script>"
         ...
         "function toggleTraceReplay() { if (traceReplayTimer) { stopTraceReplay(); render(); return; } if (!stepTraceMap(1, true)) return; traceReplayTimer = setInterval(() => stepTraceMap(1, false), 900); render(); }"
         "function renderTraceMapControls(events) { return `<div class='trace-map-controls'><button type='button' data-trace-map-step='prev'>Prev</button><button type='button' data-trace-map-play>${traceReplayTimer ? 'Pause' : 'Play'}</button><button type='button' data-trace-map-step='next'>Next</button><span class='count'>${events.length} map events</span></div>`; }"
         "function renderTraceMap(rows) { const allEvents = traceMapRows(rows); const events = allEvents.slice(-32); if (!events.length) return ''; const width = Math.max(520, 92 + (events.length - 1) * 74); const lanes = [['worlds', 42], ['rules', 86], ['problems', 132]]; const laneLines = lanes.map(([label, y]) => `<line class='trace-map-lane' x1='42' y1='${y}' x2='${width - 18}' y2='${y}'></line><text class='trace-map-lane-label' x='6' y='${y + 3}'>${label}</text>`).join(''); const links = events.slice(1).map((event, index) => { const previous = events[index]; const x1 = 46 + index * 74; const y1 = traceMapLane(previous).y; const x2 = 46 + (index + 1) * 74; const y2 = traceMapLane(event).y; const focused = [previous.id, event.id].includes(state.traceFocusId); return `<path class='trace-map-link ${focused ? 'focused' : ''}' d='M ${x1 + 32} ${y1} C ${x1 + 48} ${y1}, ${x2 - 48} ${y2}, ${x2 - 32} ${y2}'></path>`; }).join(''); return `${renderTraceMapControls(allEvents)}<div class='trace-map'><svg viewBox='0 0 ${width} 172' role='img' aria-label='Trace map'>${laneLines}${links}${events.map(renderTraceMapEvent).join('')}</svg></div>`; }"
         ...
         "inspector.addEventListener('change', event => { const traceControl = event.target.closest('[data-trace-control]'); if (traceControl) { stopTraceReplay(); if (traceControl.dataset.traceControl === 'family') state.traceFamily = traceControl.value; if (traceControl.dataset.traceControl === 'kind') state.traceKind = traceControl.value; if (traceControl.dataset.traceControl === 'scope') state.traceScope = traceControl.value; render(); return; } const xrefControl = event.target.closest('[data-xref-control]'); if (!xrefControl) return; if (xrefControl.dataset.xrefControl === 'operation') state.xrefOperation = xrefControl.value; if (xrefControl.dataset.xrefControl === 'slot') state.xrefSlot = xrefControl.value; if (xrefControl.dataset.xrefControl === 'target') state.xrefTarget = xrefControl.value; render(); });"
         "inspector.addEventListener('click', event => { const traceMapStep = event.target.closest('[data-trace-map-step]'); if (traceMapStep) { stopTraceReplay(); stepTraceMap(traceMapStep.dataset.traceMapStep === 'next' ? 1 : -1); return; } const traceMapPlay = event.target.closest('[data-trace-map-play]'); if (traceMapPlay) { toggleTraceReplay(); return; } const traceJump = event.target.closest('[data-trace-jump]'); if (traceJump) { stopTraceReplay(); jumpTrace(traceJump.dataset.traceJump === 'next' ? 1 : -1); return; } const ref = event.target.closest('[data-ref-kind]'); if (ref) { stopTraceReplay(); selectReference(ref.dataset.refKind, ref.dataset.refName, ref.dataset.refKb || null); return; } const traceEvent = event.target.closest('[data-trace-id]'); if (!traceEvent) return; stopTraceReplay(); state.traceFocusId = Number(traceEvent.dataset.traceId); render(); });"
