...
(defun write-viewer-head (stream title)
  ...
  (write-html-lines
   ...
   (list "<style>"
         ...
         ".trace-controls { display: grid; grid-template-columns: minmax(0, 1fr) minmax(0, 1fr) auto; gap: 8px; align-items: end; margin-bottom: 10px; }"
...
(defun write-viewer-script (stream)
  (write-html-lines
   ...
   (list "</script>"
         ...
         "const state = { view: 'units', selected: null, query: '', zoom: 1, viewBox: null, focusSelected: false, traceKind: 'all', traceScope: 'selected' };"
         ...
         "function filteredTraces(graph, detail) { return (DATA.details.traces || []).filter(event => (state.traceKind === 'all' || traceKind(event) === state.traceKind) && traceInSelectedScope(event, graph, detail)); }"
         "function renderTraceControls(rows, total) { const kindOptions = ['all', ...traceKinds()].map(kind => `<option value='${esc(kind)}' ${state.traceKind === kind ? 'selected' : ''}>${esc(kind === 'all' ? 'all kinds' : kind)}</option>`).join(''); const scopeOptions = [['selected', 'selected node'], ['all', 'all events']].map(([value, label]) => `<option value='${value}' ${state.traceScope === value ? 'selected' : ''}>${label}</option>`).join(''); return `<div class='trace-controls'><label>Kind<select data-trace-control='kind'>${kindOptions}</select></label><label>Scope<select data-trace-control='scope'>${scopeOptions}</select></label><span class='count'>${rows.length}/${total}</span></div>`; }"
         "function traceTitleHtml(event) { const kind = traceKind(event); if (kind === 'rule-fire') return `Rule fired: ${refButton('unit', event.rule, DATA.units.kb)}`; if (kind === 'rule-match') return `Rule matched: ${refButton('unit', event.rule, DATA.units.kb)}`; if (kind === 'agenda') return `Agenda: ${refButton('unit', event.ruleClass, DATA.units.kb)}`; if (kind === 'world-create') return `World created: ${refButton('world', event.world)}`; if (kind === 'world-branch') return `World branch: ${refButton('world', event.world)}`; if (kind === 'world-slot-write') return `World slot write: ${refButton('unit', event.unit, DATA.units.kb)}.${esc(event.slot || 'NIL')}`; if (kind === 'nogood') return `Nogood: ${refButton('unit', event.rule, DATA.units.kb)}`; if (kind === 'contradiction') return `Contradiction: ${refButton('world', event.world)}`; return esc(event.kind || 'Trace'); }"
         "function traceMetaHtml(event) { const parts = []; if (event.world) parts.push(`world ${refButton('world', event.world)}`); if (event.parent) parts.push(`parent ${refButton('world', event.parent)}`); if (event.rule && !traceKind(event).includes('rule')) parts.push(`rule ${refButton('unit', event.rule, DATA.units.kb)}`); if (event.action) parts.push(`action <span class='code-text'>${esc(traceValueText(event.action))}</span>`); if (event.proposition) parts.push(`proposition <span class='code-text'>${esc(traceValueText(event.proposition))}</span>`); const bindings = traceBindings(event); if (bindings) parts.push(`<span class='code-text'>${esc(bindings)}</span>`); if (event.message) parts.push(esc(event.message)); return parts.map(part => `<span class='trace-piece'>${part}</span>`).join(''); }"
         "function renderTraceEvent(event) { const meta = traceMetaHtml(event); return `<div class='trace-event ${traceBad(event) ? 'bad' : ''}'><strong>${traceTitleHtml(event)}</strong>${meta ? `<div class='trace-meta'>${meta}</div>` : ''}</div>`; }"
         ...
         "function renderTracePane(graph, detail) { const all = DATA.details.traces || []; const filtered = filteredTraces(graph, detail); const rows = filtered.slice(-40).reverse(); return `<section class='detail-section'><h3>Trace</h3>${renderTraceControls(filtered, all.length)}${renderTraceGraph(filtered)}${rows.length ? `<div class='trace-list'>${rows.map(renderTraceEvent).join('')}</div>` : `<p class='empty'>No trace events</p>`}</section>`; }"
         ...
         "inspector.addEventListener('change', event => { const control = event.target.closest('[data-trace-control]'); if (!control) return; if (control.dataset.traceControl === 'kind') state.traceKind = control.value; if (control.dataset.traceControl === 'scope') state.traceScope = control.value; render(); });"
