...
(defun write-viewer-script (stream)
  (write-html-lines
   ...
   (list "</script>"
         ...
         "function renderTraceMap(rows) { const events = rows.filter(traceMapEventP).slice(-32); if (!events.length) return ''; const width = Math.max(520, 92 + (events.length - 1) * 74); const lanes = [['worlds', 42], ['rules', 86], ['problems', 132]]; const laneLines = lanes.map(([label, y]) => `<line class='trace-map-lane' x1='42' y1='${y}' x2='${width - 18}' y2='${y}'></line><text class='trace-map-lane-label' x='6' y='${y + 3}'>${label}</text>`).join(''); const links = events.slice(1).map((event, index) => { const previous = events[index]; const x1 = 46 + index * 74; const y1 = traceMapLane(previous).y; const x2 = 46 + (index + 1) * 74; const y2 = traceMapLane(event).y; const focused = [previous.id, event.id].includes(state.traceFocusId); return `<path class='trace-map-link ${focused ? 'focused' : ''}' d='M ${x1 + 32} ${y1} C ${x1 + 48} ${y1}, ${x2 - 48} ${y2}, ${x2 - 32} ${y2}'></path>`; }).join(''); return `<div class='trace-map'><svg viewBox='0 0 ${width} 172' role='img' aria-label='Trace map'>${laneLines}${links}${events.map(renderTraceMapEvent).join('')}</svg></div>`; }"
