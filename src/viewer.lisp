...
(defun write-viewer-script (stream)
  (write-html-lines
   ...
   (list "</script>"
         ...
         "function traceDetailHtml(rows) { const event = rows.find(candidate => candidate.id === state.traceFocusId); if (!event) return ''; const fields = Object.entries(event).filter(([, value]) => traceDetailValueP(value)); const body = fields.map(([key, value]) => `<dt>${esc(key)}</dt><dd><span class='code-text'>${esc(traceValueText(value))}</span></dd>`).join(''); return `<div class='trace-detail'><strong>Focused Trace ${esc(event.id)}</strong><dl class='trace-detail-grid'>${body}</dl></div>`; }"
