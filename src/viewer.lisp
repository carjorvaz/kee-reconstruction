...
(defun write-viewer-script (stream)
  (write-html-lines
   ...
   (list "</script>"
         ...
         "const state = { view: 'units', selected: null, query: '', zoom: 1, viewBox: null, focusSelected: false, traceFamily: 'all', traceKind: 'all', traceScope: 'selected', xrefOperation: 'all', xrefSlot: 'all', xrefTarget: 'all' };"
         ...
         "function filteredTraces(graph, detail) { return (DATA.details.traces || []).filter(event => (state.traceFamily === 'all' || traceFamily(event) === state.traceFamily) && (state.traceKind === 'all' || traceKind(event) === state.traceKind) && traceInSelectedScope(event, graph, detail)); }"
         "function renderTraceControls(rows, total) { const familyOptions = [['all', 'all families'], ['methods', 'methods'], ['rules', 'rules'], ['worlds', 'worlds'], ['problems', 'problems']].map(([value, label]) => `<option value='${value}' ${state.traceFamily === value ? 'selected' : ''}>${label}</option>`).join(''); const kindOptions = ['all', ...traceKinds()].map(kind => `<option value='${esc(kind)}' ${state.traceKind === kind ? 'selected' : ''}>${esc(kind === 'all' ? 'all kinds' : kind)}</option>`).join(''); const scopeOptions = [['selected', 'selected node'], ['all', 'all events']].map(([value, label]) => `<option value='${value}' ${state.traceScope === value ? 'selected' : ''}>${label}</option>`).join(''); return `<div class='trace-controls'><label>Family<select data-trace-control='family'>${familyOptions}</select></label><label>Kind<select data-trace-control='kind'>${kindOptions}</select></label><label>Scope<select data-trace-control='scope'>${scopeOptions}</select></label><span class='count'>${rows.length}/${total}</span></div>`; }"
