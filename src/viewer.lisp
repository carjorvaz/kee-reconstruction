...
(defun write-viewer-script (stream)
  (write-html-lines
   ...
   (list "</script>"
         ...
         "function agendaEffectP(event) { return ['world-slot-write', 'world-branch', 'nogood', 'contradiction'].includes(traceKind(event)); }"
         "function agendaEffectEvents(fire, rows) { const start = rows.findIndex(event => event.id === fire.id); if (start < 0) return []; const effects = []; for (let index = start + 1; index < rows.length; index += 1) { const event = rows[index]; const kind = traceKind(event); if (agendaTraceP(event)) break; if (agendaEffectP(event)) effects.push(event); } return effects; }"
         "function agendaEffectText(event) { const kind = traceKind(event); if (kind === 'world-slot-write') return `${event.unit}.${event.slot}: ${traceValueText(event.oldValues)} -> ${traceValueText(event.newValues)}`; if (kind === 'world-branch') return `${event.parent || 'NIL'} -> ${event.world || 'NIL'} ${traceValueText(event.fact)}`; if (kind === 'nogood') return `${event.world || 'NIL'} ${traceValueText(event.proposition)}`; if (kind === 'contradiction') return `${event.world || 'NIL'} ${traceValueText(event.proposition)}`; return JSON.stringify(event); }"
