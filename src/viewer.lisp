...
(defun write-viewer-script (stream)
  (write-html-lines
   ...
   (list "</script>"
         ...
         "function renderAgendaPane(rows) { const groups = agendaGroups(rows).slice(-5).reverse(); return groups.length ? `<div class='agenda-board'>${groups.map(renderAgendaGroup).join('')}</div>` : ''; }"
