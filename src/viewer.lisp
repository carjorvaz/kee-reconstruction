...
(defun write-viewer-head (stream title)
  ...
  (write-html-lines
   ...
   (list "<style>"
         ...
         ".dim { opacity: 0.22; }"
         "aside { min-width: 0; overflow: hidden; padding: 14px; background: var(--panel); border-left: 1px solid var(--line); display: grid; grid-template-rows: minmax(160px, 34vh) minmax(0, 1fr); gap: 12px; }"
  ...
  (write-html-lines
   ...
   (list "<nav class='tabs' aria-label='Graph views'>"
         ...
         "<div class='browser-head'><h2>Browser</h2><span id='node-count' class='count'></span></div>"
